...
vosk_asr_open(switch_asr_handle_t *ah, const char *codec, int rate, const char *dest, switch_asr_flag_t *flags) {
...
static switch_status_t vosk_asr_load_grammar(switch_asr_handle_t *ah, const char *grammar, const char *name) {
	...
	if (!strcmp(grammar, "default")) {
		vosk->server_url = switch_core_strdup(ah->memory_pool, globals.server_url);
	} else {
		vosk->server_url = switch_core_strdup(ah->memory_pool, grammar);
	}

	ks_json_add_string_to_object(req, "url", vosk->server_url);

	if (kws_connect_ex(&vosk->ws, req, KWS_BLOCK | KWS_CLOSE_SOCK, globals.ks_pool, NULL, 50) != KS_STATUS_SUCCESS) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "Websocket connect to %s failed\n", vosk->server_url);
		ks_json_delete(&req);
